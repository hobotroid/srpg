...
package  {
	...
	public class Pixelator extends Sprite{
		...
		public function Pixelator(_source:DisplayObject, _dest:DisplayObject, _cutoff:int):void {	
			...
			copyData.draw(_source, _source.transform.matrix, _source.transform.colorTransform);
			_src.addChild(new Bitmap(copyData));
			copyData = new BitmapData(_dest.width, _dest.height, true, 0);
			copyData.draw(_dest, _dest.transform.matrix, _dest.transform.colorTransform);
		...
		public function reconfigure(_source:DisplayObject, _dest:DisplayObject, _cutoff:int):void {	
		...
		private function pixelateIn(e:Event):void {
			...
			_scaleMatrix = _dst.transform.matrix;
			_scaleMatrix.scale(1/_pixelSize, 1/_pixelSize);
			_bitmapProcess.draw(_dst, _scaleMatrix, _dst.transform.colorTransform);
		...
		private function pixelateOut(e:Event):void {
			...
			_scaleMatrix = _src.transform.matrix;
			_scaleMatrix.scale(1/_pixelSize, 1/_pixelSize);
			_bitmapProcess.draw(_src, _scaleMatrix, _src.transform.colorTransform);
			_bitmap.bitmapData = _bitmapProcess;
